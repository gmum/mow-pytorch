...
def mmd_wrapper(sample_qz: torch.Tensor, noise_creator: NoiseCreator) -> torch.Tensor:
    assert len(sample_qz.size()) == 2

    N = sample_qz.size(0)
    sample_pz = noise_creator.create(N, sample_qz.device)

    return mmd(sample_qz, sample_pz)


def mmd(sample_qz: torch.Tensor, sample_pz: torch.Tensor) -> torch.Tensor:
    assert len(sample_qz.size()) == 2
